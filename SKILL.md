...
metadata: {"openclaw":{"requires":{"anyBins":["draw.io","drawio"]},"emoji":"📐","os":["darwin","linux","win32"],"install":[{"id":"brew-drawio","kind":"brew","formula":"drawio","bins":["drawio"],"label":"Install draw.io via Homebrew","os":["darwin"]}]},"hermes":{"tags":["drawio","diagram","flowchart","architecture","visualization","uml"],"category":"design","requires_tools":["drawio","draw.io"],"related_skills":["mermaid","excalidraw","plantuml"]},"author":"Agents365-ai","version":"2.0.0"}
---
...
| `scripts/validate.py` | **MANDATORY after every .drawio generation** — deterministic structural lint (dangling edges, dup/reserved ids, broken parents, vertex overlap, edge-through-vertex, edge crossing). Must pass with 0 errors and 0 warnings before proceeding to export |
| `references/citadel-km-rendering.md` | The diagram is uploaded to 学城 / km.sankuai.com / citadel (via `uploadDrawioToDocument`) — 学城's renderer differs from local draw.io (no HTML tags, ignores `&#xa;` line breaks, boxes `text;` cells unless `fillColor=none;strokeColor=none`); includes the fetchDrawio→Chrome-headless verification loop |
...
2. **Plan** — write the diagram's one-sentence message first (it becomes the title), then identify shapes, relationships, layout (LR or TB), group by tier/layer. For non-trivial diagrams (data charts, trade-offs, multi-view architectures), read `references/visual-communication.md` to pick the right comparison type / concept-visual grammar before generating
3. **Generate** — write `.drawio` XML file to disk. **Always read `references/pitfalls.md` before writing XML** for any multi-container / multi-edge diagram — it contains battle-tested geometry rules (waypoint alignment, arrowhead stubs, edge routing, swimlane padding, cascade updates) that prevent the most common layout failures. Default output dir is the user's working dir; if the user specified an output path or directory (e.g. `./artifacts/`, `docs/images/`), use that instead — `mkdir -p` the target dir first. Apply the same dir choice to PNG/SVG/PDF exports in steps 4 and 7.
3b. **Validate gate (MANDATORY)** — run `python3 <this-skill-dir>/scripts/validate.py <name>.drawio` immediately after generating. This is a **hard gate**, not optional. If there are any errors or warnings, you MUST fix the XML and re-run validate.py. **Loop until 0 errors AND 0 warnings**, with these exceptions that can be accepted:
   - **End node inner/outer circle overlap** — by-design for UML end symbols, always ignore
   - **`edges cross` in ER diagrams** — many-to-many relationship lines inherently cross; accept after 2 fix attempts if the crossings cannot be eliminated by repositioning tables
