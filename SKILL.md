...
3b. **Validate gate (MANDATORY)** — run `python3 <this-skill-dir>/scripts/validate.py <name>.drawio` immediately after generating. This is a **hard gate**, not optional. If there are any errors or warnings (especially `edge routes through vertex`, `vertices overlap`, `edges cross`), you MUST fix the XML and re-run validate.py. **Loop until 0 errors AND 0 warnings.** Common fixes: add waypoints to route edges around nodes, use multiple End nodes to avoid long-distance routing, shift nodes to eliminate overlap. Max 3 validate-fix iterations — if issues persist, proceed but report them.
